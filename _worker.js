const Version = '2026-08-18 12:00:00';
...
///////////////////////////////////////////////////////子账号（多用户）管理///////////////////////////////////////////////
const 子用户缓存TTL毫秒 = 60000, 子用户数量上限 = 100;
const 子用户UUID格式 = /^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-4[0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}$/;
let 缓存子用户列表 = null;

function 子用户有效(用户) {
	if (!用户 || typeof 用户.uuid !== 'string' || !子用户UUID格式.test(用户.uuid)) return false;
	if (用户.enabled === false) return false;
	if (用户.expiry) {
		const 到期时间 = Date.parse(用户.expiry);
		if (Number.isFinite(到期时间) && 到期时间 < Date.now()) return false;
	}
	return true;
}

async function 读取子用户列表(env, 强制刷新 = false) {
	const 当前毫秒 = Date.now();
	if (!强制刷新 && 缓存子用户列表 && 当前毫秒 - 缓存子用户列表.时间 < 子用户缓存TTL毫秒) return 缓存子用户列表.数据;
	let 列表 = [];
	try {
		if (env && env.KV && typeof env.KV.get === 'function') {
			const 原文 = await env.KV.get('users.json');
			if (原文) {
				const 解析 = JSON.parse(原文);
				if (Array.isArray(解析?.users)) 列表 = 解析.users;
			}
		}
	} catch (error) {
		console.error(`读取users.json出错: ${error.message}`);
	}
	缓存子用户列表 = { 数据: 列表, 时间: 当前毫秒 };
	重建流量配额映射();
	流量配额超限集合.clear();// 列表刷新后重新评估配额（管理员可能上调额度）
	return 列表;
}

async function 保存子用户列表(env, 列表) {
	await env.KV.put('users.json', JSON.stringify({ users: 列表 }, null, 2));
	缓存子用户列表 = { 数据: 列表, 时间: Date.now() };
	重建流量配额映射();
	流量配额超限集合.clear();
}

function 规范化子用户列表(输入列表, 主UUID) {
	if (!Array.isArray(输入列表)) throw new Error('请求体必须为 users 数组');
	if (输入列表.length > 子用户数量上限) throw new Error(`子账号数量超过上限 ${子用户数量上限}`);
	const 已用UUID = new Set([String(主UUID || '').toLowerCase()]);
	const 结果列表 = [];
	for (const 条目 of 输入列表) {
		if (!条目 || typeof 条目 !== 'object' || Array.isArray(条目)) throw new Error('子账号条目格式无效');
		let UUID文本 = String(条目.uuid || '').trim().toLowerCase();
		if (!UUID文本) UUID文本 = crypto.randomUUID();
		if (!子用户UUID格式.test(UUID文本)) throw new Error(`无效的UUID格式: ${UUID文本}`);
		if (已用UUID.has(UUID文本)) throw new Error(`UUID 重复或与主账号冲突: ${UUID文本}`);
		已用UUID.add(UUID文本);
		let 到期时间 = null;
		if (条目.expiry != null && 条目.expiry !== '') {
			到期时间 = String(条目.expiry);
			if (!Number.isFinite(Date.parse(到期时间))) throw new Error(`无效的到期时间: ${到期时间}`);
		}
		let 配额GB = null;
		if (条目.quotaGB != null && 条目.quotaGB !== '' && Number(条目.quotaGB) !== 0) {
			配额GB = Number(条目.quotaGB);
			if (!Number.isFinite(配额GB) || 配额GB < 0 || 配额GB > 10240) throw new Error(`无效的流量额度: ${条目.quotaGB}（0 或留空表示不限，上限 10240GB）`);
			配额GB = Math.round(配额GB * 100) / 100;
		}
		结果列表.push({
			name: String(条目.name ?? UUID文本).slice(0, 32),
			uuid: UUID文本,
			enabled: 条目.enabled === false ? false : true,
			createdAt: 条目.createdAt || new Date().toISOString(),
			expiry: 到期时间,
			remark: String(条目.remark || '').slice(0, 100),
			quotaGB: 配额GB,
		});
	}
	return 结果列表;
...
async function 获取候选UUID集(env, 主UUID) {
	const 候选集 = new Set([String(主UUID || '').toLowerCase()]);
	try {
		const 子用户列表 = await 读取子用户列表(env);
		for (const 用户 of 子用户列表) {
			if (子用户有效(用户)) 候选集.add(用户.uuid.toLowerCase());
		}
	} catch (error) { }
	return 候选集;
}

async function 匹配子用户订阅TOKEN(env, host, 请求TOKEN) {
	if (!请求TOKEN) return null;
	try {
		const 子用户列表 = await 读取子用户列表(env);
		for (const 用户 of 子用户列表) {
			if (!子用户有效(用户)) continue;
			if (请求TOKEN === await MD5MD5(host + 用户.uuid)) return 用户;
		}
	} catch (error) { }
	return null;
}

// 链式代理路径 /video/ 使用各自账号的 UUID 加密，需逐个候选尝试解密
async function 反代参数获取多候选(url, 候选UUID集, 默认反代IP, 默认反代兜底) {
	const 主UUID = 候选UUID集?.values().next().value;
	if (候选UUID集 && 候选UUID集.size > 1 && /\/video\//i.test(decodeURIComponent(url.pathname))) {
		for (const 候选 of 候选UUID集) {
			const 上下文 = await 反代参数获取(url, 候选, 默认反代IP, 默认反代兜底);
			if (上下文.反代IP === '链式代理') return 上下文;
		}
	}
	return 反代参数获取(url, 主UUID, 默认反代IP, 默认反代兜底);
}

function 命中任意叉HTTP特征(request, url, 候选UUID集) {
	for (const 候选 of 候选UUID集 || []) {
		const { 头, 键 } = 获取叉HTTPPadding标识(候选);
		if (request.headers.get(头) || url.searchParams.get(键)) return true;
	}
	return false;
}
///////////////////////////////////////////////////////子账号流量统计与配额///////////////////////////////////////////////
...
let 流量配额映射缓存 = null;// Map uuid -> 配额字节（null 表示不限）
...
function 当前周期结束秒() {
	const [年, 月] = 当前流量周期().split('-').map(Number);
	return Math.floor((Date.UTC(年, 月, 1) - 8 * 3600 * 1000) / 1000);
}

function 重建流量配额映射() {
	流量配额映射缓存 = new Map();
	if (!缓存子用户列表) return;
	for (const 用户 of 缓存子用户列表.数据) {
		if (!用户?.uuid) continue;
		const GB = Number(用户.quotaGB);
		流量配额映射缓存.set(用户.uuid, (子用户有效(用户) && Number.isFinite(GB) && GB > 0) ? Math.round(GB * 1073741824) : null);
	}
}

async function 读取流量记录(env, 强制刷新 = false) {
	const 传入env = env || 运行环境引用;
	const 当前毫秒 = Date.now();
	if (!强制刷新 && 流量记录缓存 && 当前毫秒 - 流量记录缓存.时间 < 流量记录缓存TTL毫秒) return 流量记录缓存;
	let 记录 = { period: 当前流量周期(), users: {} };
	try {
		if (传入env?.KV?.get) {
			const 原文 = await 传入env.KV.get('traffic.json');
			if (原文) {
				const 解析 = JSON.parse(原文);
				if (解析?.period === 当前流量周期() && 解析.users && typeof 解析.users === 'object') 记录 = { period: 解析.period, users: 解析.users };
			}
		}
	} catch (error) {
		console.error(`读取traffic.json出错: ${error.message}`);
	}
	if (流量记录缓存 && 流量记录缓存.period !== 记录.period) 流量配额超限集合.clear();// 跨月自动解禁
	流量记录缓存 = { ...记录, 时间: 当前毫秒 };
	return 流量记录缓存;
}

async function 获取用户配额字节(env, uuid) {
	if (!uuid) return null;
	await 读取子用户列表(env);
	return 流量配额映射缓存?.get(String(uuid).toLowerCase()) ?? null;
}

function 用户周期流量(uuid) {
	if (!uuid) return 0;
	const 存量 = 流量记录缓存?.users?.[uuid];
	const 增量 = 流量统计缓存.get(uuid);
	return (存量?.up || 0) + (存量?.down || 0) + (增量?.up || 0) + (增量?.down || 0);
}

function 注册活跃连接(uuid, 清理函数) {
	if (!uuid) return () => { };
	let 集合 = 活跃连接注册表.get(uuid);
	if (!集合) {
		集合 = new Set();
		活跃连接注册表.set(uuid, 集合);
	}
	...
	return () => {
		...
		已注销 = true;
		集合.delete(清理函数);
		if (!集合.size) 活跃连接注册表.delete(uuid);
	};
}

function 切断用户连接(uuid) {
	const 集合 = 活跃连接注册表.get(uuid);
	if (!集合) return;
	log(`[流量] 子账号 ${uuid} 配额超限，切断其 ${集合.size} 个活跃连接`);
	for (const 清理 of [...集合]) {
		try { 清理() } catch (e) { }
	}
}

function 记账流量(uuid, 字节数, 方向上行) {
	if (!uuid || !Number.isFinite(字节数) || 字节数 <= 0) return;
	if (!运行环境引用?.KV?.put) return;// 未绑定 KV 时无法持久化，跳过记账以免内存无限增长
	const 条目 = 流量统计缓存.get(uuid) || { up: 0, down: 0 };
	if (方向上行) 条目.up += 字节数;
	else 条目.down += 字节数;
	流量统计缓存.set(uuid, 条目);
	流量未刷新字节 += 字节数;
	安排流量刷新();
	if (流量未刷新字节 >= 流量立即刷新阈值) void 刷新流量到KV().catch(() => { });
	const 配额 = 流量配额映射缓存?.get(uuid);
	if (配额 != null && 用户周期流量(uuid) >= 配额 && !流量配额超限集合.has(uuid)) {
		流量配额超限集合.add(uuid);
		切断用户连接(uuid);
	}
}
...
function 刷新流量到KV() {
	...
	流量刷新任务 = (async () => {
		...
		try {
			...
			for (const [uuid, 增量] of 快照) {
				if (!增量.up && !增量.down) continue;
				const 条目 = 记录.users[uuid] || (记录.users[uuid] = { up: 0, down: 0 });
				条目.up = (条目.up || 0) + 增量.up;
				条目.down = (条目.down || 0) + 增量.down;
...
body { margin: 0; font-family: system-ui, -apple-system, "Segoe UI", Roboto, "PingFang SC", sans-serif; background: #0f1420; color: #e6e9f0; }
header { display: flex; align-items: center; gap: 16px; padding: 14px 24px; background: #161d2e; border-bottom: 1px solid #263049; }
header h1 { font-size: 18px; margin: 0; }
header a { color: #7ab0ff; text-decoration: none; font-size: 14px; }
header .count { color: #9fb0cc; font-size: 13px; }
main { padding: 20px 24px; }
.toolbar { display: flex; gap: 10px; align-items: center; margin-bottom: 14px; flex-wrap: wrap; }
#status { font-size: 13px; color: #9fb0cc; }
#status.ok { color: #5fd08a; }
#status.err { color: #ff7a7a; }
button { background: #2456c4; color: #fff; border: 0; border-radius: 6px; padding: 7px 13px; font-size: 13px; cursor: pointer; white-space: nowrap; }
button:hover { background: #2f66d8; }
...
th { background: #1b2438; color: #9fb0cc; white-space: nowrap; }
td input[type="text"], td input[type="datetime-local"] { width: 100%; background: #0f1420; border: 1px solid #2c3a57; border-radius: 5px; color: #e6e9f0; padding: 5px 7px; font-size: 13px; }
.uuid { font-family: ui-monospace, Menlo, Consolas, monospace; font-size: 12px; color: #9fb0cc; word-break: break-all; }
.tag { display: inline-block; padding: 1px 7px; border-radius: 4px; font-size: 11px; margin-left: 6px; }
.tag.off { background: #4a3320; color: #f0b45f; }
.tag.expired { background: #4a2030; color: #ff8aa0; }
dialog { background: #161d2e; color: #e6e9f0; border: 1px solid #2c3a57; border-radius: 10px; padding: 20px; min-width: 320px; }
dialog::backdrop { background: rgba(0,0,0,.55); }
dialog h2 { margin: 0 0 14px; font-size: 16px; }
label { display: block; font-size: 13px; color: #9fb0cc; margin: 10px 0 4px; }
dialog input { width: 100%; background: #0f1420; border: 1px solid #2c3a57; border-radius: 5px; color: #e6e9f0; padding: 7px 9px; font-size: 13px; }
.dialogbtns { display: flex; gap: 10px; justify-content: flex-end; margin-top: 18px; }
.tip { font-size: 12px; color: #7484a3; margin-top: 10px; line-height: 1.7; }
</style>
</head>
<body>
<header>
	...
	<a href="/admin">← 返回主面板</a>
</header>
<main>
	<div class="toolbar">
		<button id="addBtn">＋ 新增子账号</button>
		<button id="saveBtn" class="ghost">保存全部修改</button>
		<button id="resetAllBtn" class="ghost">清空全部流量</button>
		<span id="status"></span>
	</div>
	<div class="tablewrap">
	<table>
		<thead><tr><th style="width:100px">名称</th><th style="width:260px">UUID</th><th>订阅链接</th><th style="width:170px">流量额度(GB)</th><th style="width:70px">启用</th><th style="width:170px">到期时间</th><th style="width:120px">备注</th><th style="width:150px">操作</th></tr></thead>
		<tbody id="tbody"></tbody>
	</table>
	</div>
	<p class="tip">· 子账号使用独立的 UUID 进行 VLESS / Trojan / Shadowsocks 认证，与主账号（ADMIN 密码派生）互不影响。<br>· 订阅链接格式与主账号相同，可直接导入 v2rayN / Clash / Sing-box 等客户端，客户端可显示已用流量与额度。<br>· 月流量额度按北京时间自然月自动重置；统计 TCP 上下行字节（UDP/DNS 不计）；额度用尽后新连接被拒绝、存量连接被切断。<br>· 禁用、到期、删除子账号或调整额度后，最长约 1 分钟内生效（服务端存在短暂缓存）。</p>
</main>
<dialog id="addDialog">
	<h2>新增子账号</h2>
	<form id="addForm" method="dialog">
		<label>名称</label>
		<input type="text" id="fName" maxlength="32" placeholder="例如：张三" required>
		<label>UUID（留空自动生成）</label>
		<input type="text" id="fUuid" placeholder="UUIDv4 格式，可留空">
		<label>到期时间（可选）</label>
		<input type="datetime-local" id="fExpiry">
		<label>月流量额度 GB（留空或 0 = 不限）</label>
		<input type="number" id="fQuota" min="0" max="10240" step="0.01" placeholder="如 50、100、200">
		<label>备注（可选）</label>
		<input type="text" id="fRemark" maxlength="100">
		<div class="dialogbtns">
			<button type="submit" value="ok">创建</button>
...
function fmtExpiry(v) {
	...
	var out = d.getFullYear() + '-' + p(d.getMonth() + 1) + '-' + p(d.getDate()) + 'T' + p(d.getHours()) + ':' + p(d.getMinutes());
	if (out.endsWith('T00:00')) return out.slice(0, 10);
	return out;
}
function subUrl(u) {
	return location.origin + '/sub?token=' + encodeURIComponent(u.subToken || '');
}
...
function resetTraffic(uuid, name) {
	if (!confirm('确定重置「' + (name || uuid) + '」的本月流量记录？')) return;
	setStatus('重置中…');
	postJSON('traffic.json', { action: 'reset', uuid: uuid }).then(function (res) {
		if (!res.ok || res.data.error) throw new Error(res.data.error || ('HTTP ' + res.status));
		setStatus('流量已重置', 'ok');
		load();
	}).catch(function (err) {
		setStatus('重置失败: ' + err.message, 'err');
	});
}
function resetAllTraffic() {
	if (!confirm('确定清空全部子账号的本月流量记录？')) return;
	setStatus('清空中…');
	postJSON('traffic.json', { action: 'resetAll' }).then(function (res) {
		if (!res.ok || res.data.error) throw new Error(res.data.error || ('HTTP ' + res.status));
		setStatus('全部流量已清空', 'ok');
		load();
	}).catch(function (err) {
...
function render() {
	...
	users.forEach(function (u, i) {
		...
		var uuidBtn = el('button', 'mini ghost', '复制UUID');
		uuidBtn.style.marginTop = '4px';
		uuidBtn.onclick = function () { copyText(u.uuid, uuidBtn); };
		tdUuid.appendChild(uuidBox);
		...
		var tdQuota = el('td');
		var 用量行 = el('div', 'uuid', '已用 ' + fmtGB(u.usedBytes) + ' GB' + (u.quotaBytes ? (' / ' + fmtGB(u.quotaBytes) + ' GB') : ' / 不限'));
		if (u.overQuota) 用量行.appendChild(el('span', 'tag expired', '已超额'));
		用量行.style.marginBottom = '4px';
		var quotaInput = el('input');
		quotaInput.type = 'number';
		quotaInput.min = '0';
		quotaInput.max = '10240';
		quotaInput.step = '0.01';
		quotaInput.placeholder = '不限';
		quotaInput.value = (u.quotaGB != null && isFinite(Number(u.quotaGB)) && Number(u.quotaGB) > 0) ? u.quotaGB : '';
		quotaInput.oninput = function () {
			var v = parseFloat(quotaInput.value);
			u.quotaGB = (isFinite(v) && v > 0) ? v : null;
		};
		tdQuota.appendChild(用量行);
		tdQuota.appendChild(quotaInput);

		var tdEnabled = el('td');
		var check = el('input');
		check.type = 'checkbox';
		check.checked = u.enabled !== false;
		check.onchange = function () { u.enabled = check.checked; };
		tdEnabled.appendChild(check);
		if (u.expired) tdEnabled.appendChild(el('span', 'tag expired', '已过期'));
		else if (u.enabled === false) tdEnabled.appendChild(el('span', 'tag off', '已禁用'));

		var tdExpiry = el('td');
		var expiryInput = el('input');
		expiryInput.type = 'datetime-local';
		expiryInput.value = fmtExpiry(u.expiry);
		expiryInput.oninput = function () { u.expiry = expiryInput.value || null; };
		tdExpiry.appendChild(expiryInput);

		var tdRemark = el('td');
		var remarkInput = el('input');
		remarkInput.type = 'text';
		remarkInput.maxLength = 100;
		remarkInput.value = u.remark || '';
		remarkInput.oninput = function () { u.remark = remarkInput.value; };
		...
		tr.appendChild(tdName); tr.appendChild(tdUuid); tr.appendChild(tdSub);
		tr.appendChild(tdQuota); tr.appendChild(tdEnabled); tr.appendChild(tdExpiry); tr.appendChild(tdRemark); tr.appendChild(tdOp);
		tbody.appendChild(tr);
	});
	if (!users.length) {
		var tr = document.createElement('tr');
		var td = el('td', null, '暂无子账号，点击上方「新增子账号」创建。');
		td.colSpan = 8;
		tr.appendChild(td);
		tbody.appendChild(tr);
	}
...
function load() {
	setStatus('加载中…');
	fetch('users.json', { cache: 'no-store' }).then(function (r) {
...
document.getElementById('addForm').addEventListener('submit', function (ev) {
	...
	var uuid = document.getElementById('fUuid').value.trim();
	var expiry = document.getElementById('fExpiry').value;
	var quota = parseFloat(document.getElementById('fQuota').value);
	var remark = document.getElementById('fRemark').value.trim();
	users.push({ name: name, uuid: uuid, enabled: true, createdAt: new Date().toISOString(), expiry: expiry || null, quotaGB: (isFinite(quota) && quota > 0) ? quota : null, remark: remark });
	dialog.close();
	render();
	setStatus('已添加，尚未保存，请点击「保存全部修改」', 'ok');
});
document.getElementById('saveBtn').onclick = save;
document.getElementById('resetAllBtn').onclick = resetAllTraffic;
load();
</script>
</body>
</html>`;
...
export default {
	async fetch(request, env, ctx) {
		...
		if (访问路径 === 'version') {// 版本信息接口
			...
			const 候选UUID集 = await 获取候选UUID集(env, userID);
			const 反代上下文 = await 反代参数获取多候选(url, 候选UUID集, 默认反代IP, 默认反代兜底);
			ctx.waitUntil(刷新流量到KV());
			log(`[WebSocket] 命中请求: ${url.pathname}${url.search}`);
			return await 处理WS请求(request, userID, url, 反代上下文, 候选UUID集);
		} else if (管理员密码 && !访问路径.startsWith('admin/') && 访问路径 !== 'login' && request.method === 'POST') {// gRPC/叉HTTP代理
			const 候选UUID集 = await 获取候选UUID集(env, userID);
			const 反代上下文 = await 反代参数获取多候选(url, 候选UUID集, 默认反代IP, 默认反代兜底);
			ctx.waitUntil(刷新流量到KV());
			const 命中叉HTTP特征 = 命中任意叉HTTP特征(request, url, 候选UUID集);
			if (!命中叉HTTP特征 && contentType.startsWith('application/grpc')) {
				log(`[gRPC] 命中请求: ${url.pathname}${url.search}`);
				return await 处理gRPC请求(request, userID, 反代上下文, 候选UUID集);
			}
			log(`[叉HTTP] 命中请求: ${url.pathname}${url.search}`);
			return await 处理叉HTTP请求(request, userID, 反代上下文, 候选UUID集);
		} else {
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					if (访问路径 === 'admin/log.json') {// 读取日志内容
						...
						try {
							...
							await 读取流量记录(env);
							const 含订阅链接 = await Promise.all(子用户列表.map(async 用户 => {
								const 存量 = 流量记录缓存?.users?.[用户.uuid];
								const 增量 = 流量统计缓存.get(用户.uuid) || { up: 0, down: 0 };
								const 上行 = (存量?.up || 0) + 增量.up, 下行 = (存量?.down || 0) + 增量.down;
								const 配额字节 = 流量配额映射缓存?.get(用户.uuid) ?? null;
								return {
									...用户,
									expired: !子用户有效(用户),
									subToken: await MD5MD5(host + 用户.uuid),
									upBytes: 上行,
									downBytes: 下行,
									usedBytes: 上行 + 下行,
									quotaBytes: 配额字节,
									overQuota: 配额字节 != null && 上行 + 下行 >= 配额字节,
								};
							}));
							return new Response(JSON.stringify({ host, period: 当前流量周期(), users: 含订阅链接 }, null, 2), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
						} catch (error) {
							return new Response(JSON.stringify({ error: '读取子账号失败: ' + error.message }), { status: 500, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
						}
					}
					...
					if (访问路径 === 'admin/init') {// 重置配置为默认值
						...
						if (访问路径 === 'admin/config.json') { // 保存config.json配置
						...
						} else if (访问路径 === 'admin/users.json') { // 保存子账号列表
							try {
								const 请求体 = await request.json();
								const 输入列表 = Array.isArray(请求体) ? 请求体 : 请求体?.users;
							...
							try {
								...
								const 现有 = await 读取流量记录(env, true);
								const 记录 = { period: 当前流量周期(), users: { ...(现有.users || {}) } };
								if (请求体?.action === 'resetAll') {
									记录.users = {};
									流量统计缓存.clear();
									流量未刷新字节 = 0;
									流量配额超限集合.clear();
								} else if (请求体?.action === 'reset' && 请求体.uuid) {
									const 目标UUID = String(请求体.uuid).toLowerCase();
									delete 记录.users[目标UUID];
									流量统计缓存.delete(目标UUID);
									流量配额超限集合.delete(目标UUID);
								} else {
									return new Response(JSON.stringify({ error: '无效的操作，支持 action=reset(需 uuid) 或 resetAll' }), { status: 400, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
								}
								await env.KV.put('traffic.json', JSON.stringify(记录));
								流量记录缓存 = { ...记录, 时间: Date.now() };
								ctx.waitUntil(请求日志记录(env, request, 访问IP, 'Reset_Traffic', config_JSON));
								return new Response(JSON.stringify({ success: true, message: '流量记录已重置' }), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
							} catch (error) {
								console.error('重置流量失败:', error);
								return new Response(JSON.stringify({ error: '重置流量失败: ' + error.message }), { status: 500, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
							}
					...
					const 命中子用户 = await 匹配子用户订阅TOKEN(env, host, 请求TOKEN);
					const 用户客户端请求订阅 = 请求TOKEN === 订阅TOKEN || !!命中子用户;
					...
					if (用户客户端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
						config_JSON = await 读取config_JSON(env, host, 命中子用户 ? 命中子用户.uuid : userID, UA);
...
async function 处理叉HTTP请求(request, yourUUID, 反代上下文 = {}, 候选UUID集 = null) {
	...
	const 首包 = await 读取叉HTTP首包(reader, 认证凭证);
	if (!首包) {
		try { reader.releaseLock() } catch (e) { }
		return new Response('Invalid request', { status: 400 });
	}
	if (首包.uuid && 首包.uuid !== yourUUID) {
		yourUUID = 首包.uuid;
		({ 头: 本机Padding头, 键: 本机Padding键 } = 获取叉HTTPPadding标识(yourUUID));
		log(`[叉HTTP] 命中子账号: ${yourUUID}`);
	}
	if (首包.uuid) {
		流量注销 = 注册活跃连接(yourUUID, () => 清理(new Error('流量配额已用尽')));
		if (await 流量配额拒绝(yourUUID)) {
			try { reader.releaseLock() } catch (e) { }
			...
			try {
				try {
					while (true) {
						...
						if (done) break;
						if (value?.byteLength) {
							记账流量(yourUUID, value.byteLength, true);
	...
	const 下行Promise = (async () => {
		...
		await socket.readable
			.pipeThrough(new TransformStream({
				transform(块, 控制器) {
					记账流量(yourUUID, 有效数据长度(块), false);
					控制器.enqueue(块);
				}
			}))
			.pipeTo(响应流.writable, { signal: abortController.signal });
	})();
...
async function 读取叉HTTP首包(reader, token) {
	...
	const 候选凭证 = 归一化认证凭证(token);

	const 尝试解析魏烈思首包 = (data) => {
		const length = data.byteLength;
		if (length < 18) return { 状态: 'need_more' };
		const 命中UUID = 匹配UUID列表(data, 1, 候选凭证);
		if (!命中UUID) return { 状态: 'invalid' };
...
///////////////////////////////////////////////////////////////////////gRPC传输数据///////////////////////////////////////////////
async function 处理gRPC请求(request, yourUUID, 反代上下文 = {}, 候选UUID集 = null) {
	if (!request.body) return new Response('Bad Request', { status: 400 });
	const 认证凭证 = 候选UUID集 || yourUUID;
	const reader = request.body.getReader();
	const remoteConnWrapper = { socket: null, connectingPromise: null, retryConnect: null, downlinkDrain: Promise.resolve() };
				...
				while (true) {
					...
					while (pending.byteLength >= 5) {
						...
						if (remoteConnWrapper.socket || remoteConnWrapper.connectingPromise) {
							...
							if (判断是否是木马) {
								const 解析结果 = 解析木马请求(首包bytes, 认证凭证);
								if (解析结果?.hasError) throw new Error(解析结果.message || 'Invalid trojan request');
								if (解析结果.uuid && 解析结果.uuid !== yourUUID) {
									yourUUID = 解析结果.uuid;
									log(`[gRPC] 命中子账号: ${yourUUID}`);
								}
								if (解析结果.uuid) {
									流量注销 = 注册活跃连接(yourUUID, () => 关闭连接());
									if (await 流量配额拒绝(yourUUID)) throw new Error('流量配额已用尽');
								}
								...
								判断是否是木马 = false;
								const 解析结果 = 解析魏烈思请求(首包bytes, 认证凭证);
...
function 是有效WS早期数据(bytes, token) {
	...
	const 候选凭证 = 归一化认证凭证(token);
	if (bytes.byteLength >= 18 && 匹配UUID列表(bytes, 1, 候选凭证)) return true;
	if (bytes.byteLength < 58 || bytes[56] !== 0x0d || bytes[57] !== 0x0a) return false;

	return !!匹配木马密码(bytes, 候选凭证);
}
...
///////////////////////////////////////////////////////////////////////WS传输数据///////////////////////////////////////////////
async function 处理WS请求(request, yourUUID, url, 反代上下文 = {}, 候选UUID集 = null) {
	const 认证凭证 = 候选UUID集 || yourUUID;
	...
	let 流量注销 = null;
	const 确认流量并注册 = async (uuid) => {
		if (!uuid || 流量注销) return false;
		流量注销 = 注册活跃连接(uuid, () => closeSocketQuietly(serverSock));
		if (await 流量配额拒绝(uuid)) {
			log(`[WS转发] 子账号 ${uuid} 流量配额已用尽，拒绝连接`);
			处理WS显式传输错误(new Error('流量配额已用尽'));
			return true;
		}
		return false;
	};
	...
	const 获取SS上下文 = async () => {
		...
		if (!ss初始化任务) {
			ss初始化任务 = (async () => {
				...
				const 入站候选用户 = [yourUUID, ...(认证凭证 instanceof Set ? [...认证凭证].filter(候选 => 候选 !== yourUUID) : [])];
				const 入站主密钥任务缓存 = new Map();
				const 取入站主密钥任务 = (用户UUID, config) => {
					const 缓存键 = 用户UUID + '|' + config.method;
					if (!入站主密钥任务缓存.has(缓存键)) 入站主密钥任务缓存.set(缓存键, SS派生主密钥(用户UUID, config.keyLen));
					return 入站主密钥任务缓存.get(缓存键);
				};
				const 入站状态 = {
					...
					for (const 候选用户UUID of 入站候选用户) {
						for (let offset = 0; offset <= 可扫描最大偏移; offset++) {
							for (const 加密配置 of 入站候选加密配置) {
								...
								const masterKey = await 取入站主密钥任务(候选用户UUID, 加密配置);
								...
								try {
									...
									if (候选用户UUID !== yourUUID) log(`[SS入站] 命中子账号: ${候选用户UUID}`);
	...
	const 处理SS数据 = async (chunk) => {
		...
		if (上下文.用户UUID && 上下文.用户UUID !== yourUUID) yourUUID = 上下文.用户UUID;
		if (上下文.用户UUID && await 确认流量并注册(yourUUID)) return;
	...
	const 处理WS入站数据 = async (chunk) => {
		...
		if (await 写入远端(chunk)) return;
		if (判断协议类型 === '木马') {
			const 解析结果 = 解析木马请求(chunk, 认证凭证);
			if (解析结果?.hasError) throw new Error(解析结果.message || 'Invalid trojan request');
			if (解析结果.uuid && 解析结果.uuid !== yourUUID) {
				yourUUID = 解析结果.uuid;
				log(`[WS转发] 命中子账号: ${yourUUID}`);
			}
			if (解析结果.uuid && await 确认流量并注册(yourUUID)) return;
			const { port, hostname, rawClientData, isUDP } = 解析结果;
			if (isSpeedTestSite(hostname) && 反代上下文.代理类型 === null) {
	...
	// SS 模式下禁用 sec-websocket-protocol early-data，避免把子协议值（如 "binary"）误当作 base64 数据注入首包导致 AEAD 解密失败。
		if (!SS模式禁用EarlyData && earlyDataHeader) {
			try {
				const bytes = 解码WS早期数据(earlyDataHeader, 认证凭证);
...
function 解析木马请求(buffer, passwordPlainText) {
	...
	const 命中UUID = 匹配木马密码(data, 归一化认证凭证(passwordPlainText));
	if (!命中UUID) return { hasError: true, message: "invalid password" };
...
// 凭证归一化：兼容单 UUID 字符串与多用户 Set/Array，命中后用于归属与回包密钥
function 归一化认证凭证(token) {
	if (typeof token === 'string') return [token];
	if (token instanceof Set) return [...token];
	if (Array.isArray(token)) return token.filter(项 => typeof 项 === 'string');
	return [];
}

function 匹配UUID列表(data, offset, uuid列表) {
	for (const 候选 of uuid列表) {
		if (候选 && UUID字节匹配(data, offset, 候选)) return 候选;
	}
	return null;
}

const 木马密码哈希缓存 = new Map();
function 获取木马密码哈希(密码文本) {
	let 缓存 = 木马密码哈希缓存.get(密码文本);
	if (!缓存) {
		缓存 = sha224(密码文本);
		if (木马密码哈希缓存.size >= 128) 木马密码哈希缓存.clear();
		木马密码哈希缓存.set(密码文本, 缓存);
	}
	return 缓存;
}

function 匹配木马密码(data, uuid列表) {
	for (const 候选 of uuid列表) {
		if (!候选 || data.byteLength < 56) continue;
		const 密码哈希 = 获取木马密码哈希(候选);
		let 匹配 = true;
		for (let i = 0; i < 56; i++) {
			if (data[i] !== 密码哈希.charCodeAt(i)) { 匹配 = false; break; }
		}
		if (匹配) return 候选;
	}
	return null;
...
function 解析魏烈思请求(chunk, token) {
	...
	const 命中UUID = 匹配UUID列表(data, 1, 归一化认证凭证(token));
	if (!命中UUID) return { hasError: true, message: 'Invalid uuid' };
	...
	return { hasError: false, addressType, port, hostname, isUDP, rawClientData: data.subarray(rawIndex), version, uuid: 命中UUID };
}
...
async function forwardataTCP(host, portNum, rawData, ws, respHeader, remoteConnWrapper, yourUUID, request = null, 反代上下文 = {}, 允许木马反代 = false, 木马反代首包数据 = null, 仅建立连接 = false) {
	...
	if (yourUUID) 记账流量(yourUUID, 有效数据长度(rawData), true);
	const 连接超时毫秒 = 1000;
	let 已通过代理发送首包 = false;
	const TCP连接 = 创建请求TCP连接器(request);
	...
	const 安装当前连接 = async (socket, generation, downlinkDrain, retryFunc = null) => {
		...
		connectStreams(socket, ws, 取出响应头, retryFunc, 连接仍有效, remoteConnWrapper, yourUUID).catch(err => {
			if (!连接仍有效()) return;
...
function 创建上行写入队列({ 获取写入器, 获取连接任务 = null, 释放写入器, 重试连接, 关闭连接, 名称 = '上行队列', 获取流量用户 = null }) {
	...
	const enqueue = (data, allowRetry = true, waitForFlush = false) => {
		...
		// 首包阶段的字节由 forwardataTCP 的 rawData 计数，此处仅计后续上行
		if (获取流量用户) 记账流量(获取流量用户(), chunk.byteLength, true);
...
async function connectStreams(remoteSocket, webSocket, headerData, retryFunc, isCurrentSocket = null, remoteConnWrapper = null, 流量用户UUID = null) {
	...
	try {
		if (!useBYOB) {
			while (true) {
				...
				if (!value || value.byteLength === 0) continue;
				hasData = true;
				if (流量用户UUID) 记账流量(流量用户UUID, value.byteLength, false);
