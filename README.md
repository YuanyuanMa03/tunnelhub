...
登录后台访问 **`/admin/users`** 即可打开子账号管理面板，所有日常操作都在这一个页面完成：
...
- **工具栏**
  - 「＋ 新增子账号」：弹窗填写名称（UUID 留空自动生成）、可选月流量额度（GB）、到期时间与备注；
  - 「保存全部修改」：将表格内所有修改一次性写入 KV，服务端会二次校验 UUID 格式、去重与额度范围；
  - 「清空全部流量」：一键重置所有子账号的本月流量记录，立即解禁。
- **表格（每行一个子账号）**
  - **名称 / 到期时间 / 备注**：行内直接编辑；
  - **UUID**：只读展示，可一键复制；
  - **订阅链接**：一键复制该子账号的专属订阅 URL，对方导入 v2rayN / Clash 等客户端即用，无需任何额外配置；
  - **流量额度(GB)**：上方实时显示「已用 X GB / 额度 Y GB」（超额会标注红色「已超额」标签），下方输入框直接修改额度，留空或 `0` 表示不限；
  - **启用**：开关切换，关闭（或到期）后该子账号的认证与订阅同步失效，标注「已禁用」/「已过期」；
  - **操作**：「重置流量」单独清零该用户本月用量并立即解禁；「删除」移除该子账号。

> 提示：管理页显示的是强制刷新后的数据，即时准确；但禁用 / 删除 / 调整额度的**生效**存在最长约 1 分钟的服务端缓存延迟。
...
5. **作者不通过本项目提供任何代理或网络服务**，不参与、不知晓、也不控制任何第三方的部署与运营；任何第三方基于本项目代码搭建的服务、进行的宣传或交易，均与作者无关；
6. 本项目**免费开源、不接收任何形式的捐赠或赞助**；凡以本项目名义收费、售卖、募捐的行为均系冒名，与作者无关；
7. 如本项目内容与使用者所在地区法律法规相冲突，请立即停止使用并删除；
8. 建议在测试完成后 24 小时内删除本项目相关部署。
